...
export default function HomePage() {
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="text-2xl tracking-tight">Home</h1>
      ...
      <div className="mt-8 rounded-xl border border-black/10 p-4">
        <p className="text-sm text-muted">Composer</p>
      ...
      <h2 className="mt-10 text-sm text-muted">Upcoming</h2>
      <ul className="mt-3 divide-y divide-black/10 border-y border-black/10">
        {queue.map((item) => (
          <li key={item.time} className="flex gap-6 py-4">
            <span className="w-24 shrink-0 text-sm text-muted">{item.time}</span>
            <span>{item.text}</span>
          </li>
        ))}
      </ul>
